...
class _GPdodeOptimizer:
	...
	def make_optimize_tensor(self, model, session=None, var_list=None, **kwargs):
		...
		This method builds optimization tensor and initializes all necessary 
		variables created by optimizer.
	...
	def make_optimize_action(self, model, session=None, var_list=None, **kwargs):
		...
		optimizer_tensor = self.make_optimize_tensor(
								model, session, var_list=var_list, **kwargs)
		opt = Optimization()
	...
	def minimize(self, model, session=None, var_list=None, feed_dict=None,
				 maxiter=1000, initialize=False, anchor=True, 
		...
		:param var_list: List of extra variables trained during optimization.
		:param feed_dict: Feed dictionary of tensors passed to session run method.
		:param maxiter: Number of run interation.
		:param initialize: If `True` model parameters will be re-initialized even 
			if they were initialized before for gotten session.
		:param anchor: If `True` trained variable values computed during 
			optimization at particular session will be synchronized with 
			internal parameter values.
		:param step_callback: A callback function to execute at each 
			optimization step. The callback should accept variable argument list, 
			where first argument is optimization step number.
...
def _get_registered_optimizer(name):
	tf_optimizer = _REGISTERED_TENSORFLOW_CONSTRAINED_OPTIMIZERS.get(name, None)
...
for key, train_type in tfco.__dict__.items():
	suffix = 'Optimizer'
	if key != suffix and key.endswith(suffix):
		_register_optimizer(key, train_type)

__all__ = list(_REGISTERED_TENSORFLOW_CONSTRAINED_OPTIMIZERS.keys())