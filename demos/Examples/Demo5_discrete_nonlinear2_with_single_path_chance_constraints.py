"""
Demo 5: Tutorial 5
Type: discrete models with single path chance constraints
...
x1 = x0 + f(x0,u,p).
This demo has single path chance constraints, input constraints and input change 
constraints.
Tha single path chance constraints enforce the constraint at each point for a defined
confidence, i.e, it guaranteed the value of the point will respect the constraint
in "X" % of the time, where "X" is the confidence chosen by the user.
This is similar to demo design_of_experiments_for_model_discrimination_tutorial, 
however surrogate models are not used and single path chance constraints are employed, 
...
from doepy.constraints import ControlDeltaConstraint, SingleChanceStateConstraint

# Single path chance constraint
# Confidence: 90%
d_conf = 0.90
bds = Ms[0].z_bounds
z_const = SingleChanceStateConstraint(bds,conf=d_conf)
