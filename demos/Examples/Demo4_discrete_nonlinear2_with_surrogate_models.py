"""
Demo 4: Tutorial 4
Type: discrete models - using surrogate models
...
Surrogate models are used to approximate the nonlinear models. 
The models are assumed discrete, i.e, the next prediction is based on the 
previous state value and a change funtion. Thus, x1 = x0 + f(x0,u,p).
This demo only has mean value constraints, input constraints and input change 
constraints.
This is similar to demo design_of_experiments_for_model_discrimination_tutorial.

@authors: Eduardo Schultz and Simon Olofsson
@version: 0
@first release: 10.06.2019
...
# Type of the model = GPModel
from doepy.models.discrete_time import dtGPModel

dicts   = [ M.get_candidate_dict() for M in Ms ]

# All models (M1, M2 and M3) are now in the variable "models"
models  = [ dtGPModel(CandidateWrapper(**d)) for d in dicts ]
...
# %% Training 
# This step was not necessary in Demo3 because surrogate models were not used.
# In this demo, surrogate models approximate the models M1, M2 and M3.
# Therefore, this step trains the surrogate models.
for i,model in enumerate(models):
...
from doepy.constraints import ControlDeltaConstraint, ConstantMeanStateConstraint

# Only the mean value of the constraint is used
